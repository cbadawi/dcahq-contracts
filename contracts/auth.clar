...
;; Initially, only approved users can run dca-users* function to minimize the risk of intentional slippage. In future versions, a decentralized network will take over this role.
(define-read-only (is-approved-dca-network (address principal))
  (default-to false (map-get? approved-dca-network address))
)
...
(define-public (add-approved-dca-network (new-approved-network principal))
	(begin
	(asserts! (is-owner) ERR-NOT-AUTHORIZED) 
		(ok (map-set approved-dca-network new-approved-network true))
))
